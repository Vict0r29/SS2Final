...
def protected_area():
    ...
    return render_template("main.html", token_id=token_id.get('id'))


@app.route("/paraphrasing/<params>", methods=['POST', 'GET'])
async def paraphare(params):
    if request.method == 'GET':
        id = params.strip("#")
        rows_para = await rows_value(id)
        return render_template("paraphrase.html",rows_para=rows_para)
    else:
        ...
        rows_para = await rows_value(id)
        return render_template("paraphrase.html", output_para=output_para, rows_para=rows_para)


@app.route("/plagiarism/<params>", methods=['POST', 'GET'])
async def plagiarism(params):
    if request.method == 'GET':
        id = params.strip("#")
        rows_pla = await rows_value_pla(id)
        return render_template("plagiarism.html", rows_pla=rows_pla)
    else:
        ...
        rows_pla = await rows_value_pla(id)
        return render_template("plagiarism.html", output_pla=output_pla, rows_pla=rows_pla)


@app.route("/completion/<params>", methods=['POST', 'GET'])
async def completion(params):
    if request.method == 'GET':
        id = params.strip("#")
        rows_completion = await rows_value_completion(id)
        return render_template("completion.html",rows_completion=rows_completion)
    else:
        ...
        rows_completion = await rows_value_completion(id)
        return render_template("completion.html", output_completion=output_completion, rows_completion=rows_completion)


@app.route("/grammar/<params>", methods=['POST', 'GET'])
async def grammar(params):
    if request.method == 'GET':
        id = params.strip("#")
        rows_grammar = await rows_value_grammar(id)
        return render_template("grammar.html",rows_grammar=rows_grammar)
    else:
        ...
        rows_grammar = await rows_value_grammar(id)
        return render_template("grammar.html", output_grammar=output_grammar, rows_grammar=rows_grammar)
