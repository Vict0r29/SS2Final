...
openai.api_key = "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"
...
@app.route("/paraphrasing", methods=['POST', 'GET'])
async def paraphare():
    output_para = ""
    id = session["id"].strip("#")
    ...
    return render_template("paraphrase.html",
                           ...
                           id=id,
                           rows_para=rows_para,
                           pic=session["profile_pic"],
                           name=session["name"])


@app.route("/plagiarism", methods=['POST', 'GET'])
